...
class MainActivity : AppCompatActivity() {
            ...
            tiempo?.let {
                handler.postDelayed({
                    if(primeraImagen== i){
                        ...
                        if(puntosTotal==5){
                            val ultimoPunto = AlertDialog.Builder(this)
                            ultimoPunto.setMessage("Solo queda un par ¿desea terminar la partida?")
                                .setCancelable(false)
                            ...
                            val alertDialog = ultimoPunto.create()
                            alertDialog.show()
                        }
    ...
    private fun guardarGanador():String{
        var mensaje="";
        if (puntosJugadorUno!! > puntosJugadorDos!!) {
            ...
            mensaje="¡Empate!"
        }// Determina quién ganó o si hubo empate
        return mensaje
    ...
    private fun mostrarDialogoResultado(mensaje: String) {
        ...
        val dialogView = inflater.inflate(R.layout.layout_dialog_resultado, null)
        val imageResultado = dialogView.findViewById<ImageView>(R.id.imageResultado)
        val textResultado = dialogView.findViewById<TextView>(R.id.textResultado)
        val builder = AlertDialog.Builder(this)
        if(mensaje=="¡Jugador 1 gana!"){
            imageResultado.setImageResource(R.drawable.player1wins)
            textResultado.text="$puntosJugadorUno - $puntosJugadorDos"
        }
        else if(mensaje=="¡Jugador 2 gana!"){
            imageResultado.setImageResource(R.drawable.player2wins)
            textResultado.text="$puntosJugadorUno - $puntosJugadorDos"
        }
        else{
            imageResultado.setImageResource(R.drawable.draw)
            textResultado.text="$puntosJugadorUno - $puntosJugadorDos"
        }

        builder.setView(dialogView)
            .setCancelable(false)
            .setPositiveButton("Nueva Partida") { dialog, _ ->
                dialog.dismiss()
                newGame()
            }
